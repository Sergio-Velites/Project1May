            ...
            <details className="rsvp-card" key={i}>
              ...
              <div className="rsvp-detail">
                ...
                {(() => {
                  ...
                  const seenOnly  = Array.from(seenSet).filter((id) => !caughtSet.has(id)).sort((a, b) => a - b);
                  ...
                  return (
                    <div className="pokemon-section">
                      ...
                      {caughtSorted.length > 0 && (
                        <>
                          <div style={{ fontSize: "0.6rem", color: "#999", margin: "0 0 0.35rem" }}>◆ Capturados</div>
                          <div className="pokemon-grid" style={{ marginBottom: "0.6rem" }}>
                            {caughtSorted.map((id) => (
                              <div className="pokemon-tile" key={`c-${id}`}>
                                {/* eslint-disable-next-line @next/next/no-img-element */}
                                <img src={spriteUrl(id)} alt={`Pokémon #${id}`} width={56} height={56} />
                      ...
                      {seenOnly.length > 0 && (
                        <>
                          <div style={{ fontSize: "0.6rem", color: "#999", margin: "0 0 0.35rem" }}>○ Solo vistos</div>
                          <div className="pokemon-grid">
                            {seenOnly.map((id) => (
                              <div className="pokemon-tile" key={`s-${id}`} style={{ opacity: 0.55 }}>
                                {/* eslint-disable-next-line @next/next/no-img-element */}
                                <img src={spriteUrl(id)} alt={`Pokémon #${id}`} width={56} height={56} style={{ filter: "grayscale(1)" }} />
                                <span className="pokemon-level">#{id}</span>
                              </div>
                            ))}
                          </div>
